...
message=`curl -L http://$etcd_endpoint:4001/v2/keys/$register_nats_dir/index |jq '.message' | cut -f 2 -d '"'`

if [ "$message" == "Key not found" ]; then
    etcdctl set $register_nats_dir/index 0
else
    old_index=`etcdctl get $register_nats_dir/index`
    old_index=`expr $old_index + 1`
    new_index=`etcdctl set $register_nats_dir/index $old_index`
fi
